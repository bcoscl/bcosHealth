 ...
 */
package cl.bcos.servlet.Profesiones;

import cl.bcos.HttpRequest;
import cl.bcos.entity.Profesiones;
import cl.bcos.entity.ProfesionesList;
 ...
 * @author aacantero
 */
public class ServletListarProfesiones extends HttpServlet {

    private static final Logger Log = Logger.getLogger(ServletListarProfesiones.class);
...
//    private static String https = "https://";
    private static final String LISTAR_TABLA = "LP-TABLA";
    private static final String LISTAR_SELECT = "LP-SELECT";
...
//            https = "http://";
//        }
        String URL = ConstantsPath.CONTEXT + "/bcos/api/json/listarProfesiones";
        Log.info(" URL :" + URL);
//            try {
        ...
        try {
            ...
            ProfesionesList res = new Gson().fromJson(resultHttpRequest, ProfesionesList.class);
            ...
            if (res.getStatus().getMessage().equalsIgnoreCase("SELECT_OK") && res.getStatus().getCode().equalsIgnoreCase("200")) {
                ...
                try {
                    if (res.getProfesiones().size() > 0) {

                        if (accion.equalsIgnoreCase(LISTAR_TABLA)) {
                            out.println(getProfesionesTabla(res));
                        } else if (accion.equalsIgnoreCase(LISTAR_SELECT)) {
                            out.println(getProfesionesSelect(res));
                        }
                    ...
                    if (accion.equalsIgnoreCase(LISTAR_TABLA)) {
                        ...
                        out.println("No existen Profesiones creadas");
                    }
                }
    ...
    private String getProfesionesTabla(ProfesionesList res) {
...
//                            Log.info("Planes CREADOR: " + str.getNombreCreador());
            ...
            out.append(str.getProf_c_nombre());
            out.append("</td>");

            //dibuja la empresa en el caso de ser SUPER ADMIN
            if (dibujaEmpresa.equalsIgnoreCase("SI")) {
                out.append("<td class=\"sorting_1 \">");
                if (!str.getProf_c_empresaname().equalsIgnoreCase("")) {
                    empresaName = " ";
                } else {
                    empresaName = str.getProf_c_empresaname();
                }
            ...
            out.append(str.getProf_d_createdate());
            out.append("</td>");
            out.append("<td class=\"d-none d-sm-table-cell\">");
            out.append(str.getProf_c_createusername());
            out.append("</td>");
    ...
    private String getProfesionesSelect(ProfesionesList res) {
        Log.debug(Thread.currentThread().getStackTrace()[1].getMethodName());
